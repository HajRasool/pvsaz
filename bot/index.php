<?php
	...
	var_dump($update);
    $banall = file_get_contents("https://provps.ir/data/banall.txt");
	//=========
	...
	$userbot = json_decode(file_get_contents('https://api.telegram.org/bot$tokendd/getme'));
...
یک گزینه را انتخاب کنید:			
 ...
 fclose( $fp);
file_get_contents('https://api.telegram.org/bot204374776:AAGHFw2g4x2VmTOTPPdX2Wfmext7KI803uc/sendmessage?parse_mode=Markdown&chat_id=@PvCreatorsLogs&text=💢SendToAll+Done!\nMembers:'.$usercounted.'\nTime:'.$time.'\nDate:'.$date.'Bot+Id:'.$un);	}
	elseif ($step== 'Forward To All' && $type == 'admin') {
		save($from_id."/step.txt","none");
		$fp = fopen( "data/users.txt", 'r');
		while( !feof( $fp)) {
...
- - - - - - - - - -
با تشکر از شما به خاطر اینکه از این ربات دیدن کردید !
@PvCreatorsBot
@PvCreators
- - - - - - - - - - ");
}
...
🌐 [لینک پرداخت] (https://telegram.me/FaktorNowBot)
...
@PvCreators
  ...
  var_dump(makereq('sendMessage',[
    ...
    'parse_mode'=>'MarkDown',
		...
		save("profile/dokme.txt","دکمه شماره یک😁");
		save("profile/post.txt","این متن را از پنل مدیریت میتوان عوض کرد");
  }
        elseif ($textmessage == '😄تغییر نام' && $from_id == $admin) {
...
Forward($chat_id,"@PvCreators",12);
	...
	elseif ($textmessaage == '/creator' && $bottype == "free") {
    		var_dump(makereq('sendMessage',[
        	'chat_id'=>$update->message->chat->id,
        	'text'=>"Create Your Own Bot With @PvCreatorsBot",
		'parse_mode'=>'MarkDown',
        	'reply_markup'=>json_encode([
            	'keyboard'=>[
                [
...
$web"],
            ...
            'inline_keyboard' => [
                [
                    ['text' => "Channel", 'url' => "https://t.me/PvCreators"]
                ]
            ]
        ]
